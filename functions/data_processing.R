...
generate_insert_trip <- function(input, trip_record){
  ...
  for(var_name in names(trip_record)){
    ...
    if(var_name %in% tripeditor.cols){
      ...
      # replace "" in dest_purpose_other, psrc_comment with NA to match DB value
      if(input[[paste0("data_edit-",var_name)]] == ""){
        row <- as.data.frame(NA)
      }
      else{
    ...
    # datetimes
    else if(var_name == "depart_time_timestamp"){
      depart_datetime <- combine_datetime(input[["data_edit-depart_time_timestamp_date"]],input[["data_edit-depart_time_timestamp_time"]])
      row <- as.data.frame(depart_datetime)
    }
